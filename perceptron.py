...
# SPECIFICATION: read the file optdigits.tra to build a Perceptron classifier. You will simulate a grid search, trying to find which combination of two Perceptron hyperparameters (eta0, random_state) leads you to the best prediction performance.
...
for a in n: #iterates over n
    ...
    for b in r: #iterates over r
        ...
        if(accuracy > max_Accuracy):
            ...
            print("Highest Perceptron accuracy so far: %.4f, Parameters: learning rate=%s, random_state=%s" % (max_Accuracy, str(max_n), max_r))

print("\nHighest Perceptron accuracy overall: %.4f, Parameters: learning rate=%s, random_state=%s" % (max_Accuracy, str(max_n), max_r))
